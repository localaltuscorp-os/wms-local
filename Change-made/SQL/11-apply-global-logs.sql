-- ===========================================================================
--  APPLY — Global WMS Logs (daily_sessions + activity_logs + immutability trigger)
--  Altus WMS | branch Om | source: db/migrations/0245_global_logs.sql
--  Applied to the Supabase database this branch points at on 2026-09-22.
-- ===========================================================================
--
--  WHAT THIS DOES
--    Two tables and one trigger, all additive and idempotent:
...
--      activity_logs   the immutable global log (login/logout, module & page
--                      visits, record views, creates/updates/deletes, approvals,
--                      rejections, reversals, exports, searches, filters,
--                      access-denied, configuration changes).
--      trigger         activity_logs_no_mutate — refuses UPDATE and DELETE at the
--                      database level, so the log is append-only even against a
--                      bug or a direct SQL write.
--
--  SAFETY
--    * No DROP TABLE, no DELETE, no TRUNCATE. The only DROP is
--      `drop trigger if exists` + `drop function if exists` before recreating
--      them (a trigger has no data).
--    * Every statement is `if not exists` / guarded; re-running changes nothing.
--
--  NAMING
--    The brief's activity_logs field called `timestamp` is stored as `event_at`
--    (`timestamp` is an SQL reserved word). The UI labels the column "Time".
--
--  RUN: Supabase Dashboard -> SQL Editor -> paste -> Run
--       or: psql "$DATABASE_URL" -f Change-made/SQL/11-apply-global-logs.sql
--
--  VERIFY AFTERWARDS: 12-verify-global-logs.sql
--
--  Everything below this line is db/migrations/0245_global_logs.sql, verbatim.
-- ===========================================================================
