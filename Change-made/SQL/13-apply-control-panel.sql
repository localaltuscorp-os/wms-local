-- ===========================================================================
--  APPLY — Control Panel (roles + role_permissions + employee_roles)
--  Altus WMS | branch Om | source: db/migrations/0246_control_panel.sql
--  Applied to the Supabase database this branch points at on 2026-09-22.
-- ===========================================================================
--
--  WHAT THIS DOES
--    Three additive, idempotent tables — a Roles template layer over the
--    existing permission architecture (enforcement stays in module_permissions):
--      roles             a named reusable template (e.g. "HR Admin").
--      role_permissions  one (module node, action, scope) a role grants.
--      employee_roles    the many-to-many user <-> role assignment.
--    Plus one seeded, non-deletable "Super Admin" role.
--
--  SAFETY
--    No DROP, no DELETE, no TRUNCATE. Every statement is `if not exists`; the
--    seed is guarded by `where not exists`. Re-running changes nothing.
--
--  RUN: Supabase Dashboard -> SQL Editor -> paste -> Run
--       or: psql "$DATABASE_URL" -f Change-made/SQL/13-apply-control-panel.sql
--
--  VERIFY AFTERWARDS: 14-verify-control-panel.sql
--
--  Everything below this line is db/migrations/0246_control_panel.sql, verbatim.
-- ===========================================================================
