...
export function ScopePicker({ picker, who, meId }: { picker: PickerOption[]; who: string; meId: string }) {
  ...
  return (
    <label className="inline-flex items-center gap-2 rounded-xl border border-hairline-strong bg-white px-3 py-1.5">
      ...
      <CompactSelect
        value={who}
        onChange={(v) => router.push(href({ who: v === "me" ? null : v }))}
        className="max-w-[260px] bg-transparent text-[13.5px] font-bold text-ink-strong"
        aria-label="Whose checklist"
        required
        panelWidth={260}
        // The <optgroup> headings survive the move: CompactSelect draws one per
        // run of equal `group` values, so the list still reads by Function.
        options={[
          { value: "me", label: "Only me" },
          { value: "team", label: `Full team — me and ${others} below me` },
          ...groups.flatMap((g) =>
            g.people.map((p) => ({ value: p.id, label: p.name, group: g.label })),
          ),
        ]}
      />
    </label>
