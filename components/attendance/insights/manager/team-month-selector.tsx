...
/** Month/year picker that drives the team insights `?y=&m=` query. Mirrors the
 *  attendance dashboard selector but targets `/attendance/insights/team`. */
export function TeamMonthSelector({ year, month }: { year: number; month: number }) {
  ...
  function go(y: number, m: number) {
    ...
    router.push(`/attendance/insights/team?y=${yy}&m=${mm}` as Route);
  }
  ...
  return (
    ...
    >
      <button type="button" aria-label="Previous month" onClick={() => go(year, month - 1)} className={chevronCls}>
        <ChevronLeft size={16} strokeWidth={2.4} />
      </button>
      <select value={month} aria-label="Month" onChange={(e) => go(year, Number(e.target.value))} className={selectCls}>
        {MONTH_LABELS.map((label, i) => (
          <option key={label} value={i + 1}>{label}</option>
        ))}
      </select>
      <span aria-hidden className="h-4 w-px bg-[var(--color-hairline-strong)]" />
      <select value={year} aria-label="Year" onChange={(e) => go(Number(e.target.value), month)} className={`${selectCls} tabular-nums`}>
        {years.map((y) => (
          <option key={y} value={y}>{y}</option>
        ))}
      </select>
      <button type="button" aria-label="Next month" onClick={() => go(year, month + 1)} className={chevronCls}>
