...
/**
 * Month selector for the per-employee attendance dashboard. Drives the `?y=&m=`
 * query while PRESERVING the employee id in the path — so switching months never
 * makes the viewer re-pick the person. Mirrors AttendanceMonthSelector's frosted
 * segmented-pill look and keyboard affordances.
 */
...
export function EmployeeMonthSelector({
  employeeId,
  year,
  month,
}: {
  ...
  function go(y: number, m: number) {
    ...
    router.push(
      `/attendance/insights/employee/${employeeId}?y=${yy}&m=${mm}` as Route,
    );
  }
