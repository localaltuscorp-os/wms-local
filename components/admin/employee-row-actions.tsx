...
export function EmployeeRowActions({
  ...
  return (
    <>
        ...
        <DropdownMenuContent align="end">
          ...
          {canDelete && (
            <DropdownMenuItem danger onClick={() => setDeleteOpen(true)}>
              <Trash2 size={14} />
              Offboard employee</DropdownMenuItem>
          )}
        </DropdownMenuContent>
      </DropdownMenu>
      ...
      {/*
        OFFBOARDING. The hard-delete dialog that stood here destroyed the
        employee AND every audit row pointing at them — see
        offboarding-actions.ts. It is replaced by a wizard that records the
        departure and deletes only the identity.
      */}
      <ArchiveEmployeeDialog
        ...
        employee={{
          ...
          joinedAt: employee.joinedAt ? new Date(employee.joinedAt).toISOString() : null,
        }}
        successorOptions={managerOptions.filter((o) => o.value !== employee.id)}
        canSetLegalHold={canManageAdmins}
      />
