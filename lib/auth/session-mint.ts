...
/**
 * Turn a verified Firebase ID token into this app's session cookies.
 *
 * Lifted out of app/api/auth/session/route.ts unchanged when the password check
 * moved to the server (account lockout): BOTH entry points must mint a session
 * identically, and two copies of cookie-splitting code is how they stop being
 * identical. The two callers are:
 *
 *   · /api/auth/login   — the server checked the password itself, and needs the
 *                         same cookies plus its own JSON body.
...
export async function mintSessionForIdToken(
  ...
  try {
    const res = await setAuthCookies(forwardedHeaders, {
      ...
      // setAuthCookies (here), authMiddleware (proxy.ts) and getTokens
      // (lib/auth/session.ts). They agree on the cookie NAMES, so changing it in
      // one place alone makes the other two unable to read what it wrote - which
      // fails exactly like this bug.
