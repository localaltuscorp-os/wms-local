/**
 * VALIDATE migrations 0217–0220 against the REAL schema, changing nothing.
 *
 * ── WHY THIS EXISTS ────────────────────────────────────────────────────────
 ...
 * ROLLS BACK. Postgres parses, plans and executes every statement — DDL
 * included, since it is transactional in Postgres — which means a broken
 * migration fails here, loudly, against the actual production schema. When the
 * rollback completes the database is byte-for-byte as it was.
 *
...
async function main() {
  console.log("Validating 0217–0220 against the live schema (transaction will be rolled back).\n");
  ...
  console.log("VALIDATION PASSED — all four migrations apply cleanly to the live schema.");
}
