...
export default async function OutstandingPage({ searchParams }: PageProps) {
  ...
  /* Every link on this page is built from one constant rather than typed out
     eight times, so the module can be re-homed again without hunting for
     stragglers — which is exactly how the Sales links were missed. */
  ...
  return (
    <>
      ...
      <PageShell width="full" className="outstanding-print-root">
        ...
        <OutstandingStatCards totals={dashboard.totals} sp={sp} base={base} />

        <OverdueBucketsPanel buckets={dashboard.buckets} sp={sp} base={base} />
        ...
        <PdcPanel pdc={dashboard.pdc} sp={sp} base={base} />
