                ...
                {rows.map((c) => (
                  <tr key={c.id}>
                    <td className="border-t border-hairline py-2.5 pr-3">
                      <Link href={`/billing/contracts/${c.id}` as Route} className="font-bold text-ink-strong underline-offset-2 hover:underline">
                        {c.customerName}
                      </Link>
                      <div className="flex items-center gap-1 text-[11.5px] text-ink-muted">
                        {entityName.get(c.entityId) ?? c.entityId}
                    ...
                    <td className="border-t border-hairline py-2.5 pr-3">
                      ...
                      {c.billingFrequency ? (
                        <span className="text-ink-muted"> · {c.billingFrequency === "quarterly" ? "Quarterly" : "Monthly"}</span>
                      ) : null}
                    </td>
                    <td className="border-t border-hairline py-2.5 pr-3 tabular-nums text-ink-muted">
                      {c.startDate} → {c.endDate}
                    </td>
                    <td className="border-t border-hairline py-2.5 pr-3 text-right font-bold tabular-nums">{rupees(c.totalValue)}</td>
                    <td className="border-t border-hairline py-2.5 pr-3 text-right tabular-nums">{rupees(c.billedAmount)}</td>
                    ...
                    <td className="border-t border-hairline py-2.5">
                      <span className="inline-flex rounded-pill px-2.5 py-1 text-[11.5px] font-bold" style={STATUS_STYLE[c.status]}>
